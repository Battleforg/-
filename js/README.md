...
把实现了Iterable接口，可以通过迭代器Iterator消费的对象称为“可迭代对象”。实现Iterable接口需要在内部提供一个迭代器，这个迭代器知道如何迭代对象。实现Iterable接口必须暴露一个属性作为“默认迭代器”，而且这个属性必须使用特殊的Symbol.iterator作为键。这个默认迭代器属性必须引用一个迭代器工厂函数，调用这个工厂函数必须返回一个迭代器。
